...
- **Diagrams must be editorial-grade — not Excel.** No two-bare-rectangles-and-a-dashed-line. Required elements when you draw something:
  ...
  - **Draw on a 640-unit viewBox** (author's ruling, 2026-08-29, after two failed attempts at widening instead). The prose column is boxed in by a table-of-contents rail on each side and cannot be widened: measured, the gap from the column's right edge to the right rail is a constant 49 px at 1024/1280/1440, and a CSS breakout does not widen anything — it slides the diagram *under* the rail. The rendered width is ~640 px at 1280 and ~800 px at 1440, so an 800-unit canvas shrank every font by 20–25 % (a 13 px label landed at 9.9 px, and near 5.7 px on a phone). On a 640-unit canvas the scale never drops below 1, so **a nominal size is the size the reader gets**. Minimum nominal sizes: eyebrow 14.5, headline 28, labels and annotations 13, footnotes 13 — nothing under 13. Usable content width is 576 units (32-unit margins). If a label doesn't fit, redesign the layout or cut the label; never shrink the text, and never widen the canvas to make room. Diagrams are wrapped in `.diagram-scroll`, which holds a 40rem floor and scrolls horizontally on narrow viewports rather than shrinking the type.
...
- **Concision is the point** (author's ruling, 2026-08-29): an article earns its length. The impact comes from **diagrams, numbers and blockquotes**, not from prose volume — so when a passage can be a figure, a count or a pull-quote, make it one and delete the paragraph. Target roughly 2,000-2,200 words of body per language, sources excluded. Cut before adding: no paragraph that restates the one above it, no sentence that narrates the article's own structure ("this is where most write-ups stop", "I will concede it in one sentence", "so it goes first" — see the anti-slop rule below), and no explanation of a point the diagram already carries.
...
- **Draft status.** Add `draft: true` to a post's `metadata` while it is being written. A draft is excluded from the home page, the archives, the tag pages and tag counts, the related-posts block, the RSS feeds, the sitemap, the OG images and the pre-rendered HTML — and it sends no analytics beacon. It stays **readable live** at `/posts/<slug>` (and `/en/posts/<slug>`) with an amber "brouillon" banner and a `noindex, nofollow` meta, so a work in progress can be reviewed on the real site. All drafts are listed at `/drafts` (`/en/drafts`), an unlisted page that is not in the navigation, not in the sitemap and disallowed in `robots.txt`. **Publishing is deleting the line:** remove `draft: true` and the post rejoins every listing on the next build. Note that cross-links written into already-published posts still point at a slug you flag back to draft — the flag governs the post's own publication, not other posts' bodies.
