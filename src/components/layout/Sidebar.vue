<template>
  <aside class="hidden lg:flex lg:flex-col w-80 border-r border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/50">
    <div class="flex-1 overflow-y-auto p-6">
      <!-- Top Section: Name and Theme Toggles -->
      <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-2">
          <span class="text-lg font-semibold text-gray-900 dark:text-gray-100">Damien BATTISTELLA</span>
        </div>
        <div class="flex items-center gap-2">
          <button
            @click="i18n.switchLanguage(lang === 'fr' ? 'en' : 'fr')"
            class="w-8 h-8 rounded-full bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors flex items-center justify-center text-xs font-medium text-gray-700 dark:text-gray-300"
          ...
          <button
            ...
            class="w-8 h-8 rounded-full bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors flex items-center justify-center"
            :aria-label="darkMode.isDark.value ? 'Switch to light mode' : 'Switch to dark mode'"
          >
            <i :class="darkMode.isDark.value ? 'pi pi-moon' : 'pi pi-sun'" class="text-sm text-gray-700 dark:text-gray-300"></i>
          </button>
        </div>
      </div>
      ...
      <nav class="space-y-1 mb-6">
        <RouterLink
          :to="langPrefix || '/'"
          class="flex items-center gap-3 px-3 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 rounded transition-colors"
