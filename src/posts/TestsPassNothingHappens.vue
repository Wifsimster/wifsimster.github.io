...
<hr>
<p>Ask me whether I trust the code my team's agents write and the honest answer is that the question is badly posed. I do not trust it. I also do not read it, not line by line, not any more. Neither answer tells you much.</p>
<p>What matters is what you build when both are true at once.</p>
...
<h2>The 4 layers</h2>
<p>So those properties no longer rest on anyone's attention. They rest on code that runs. 4 layers, in the order a change meets them.</p>
...
const frenchContent: PostContent = {
  ...
  html: `<div class="post-tldr"><p><strong>TL;DR</strong> En santé régulée, la qualité n'est pas un nombre de bogues. C'est un ensemble de propriétés qui doivent tenir après chaque changement : une trace d'audit opposable, une identité que personne ne peut usurper, l'accessibilité comme obligation légale, des contrats d'interopérabilité qui ne mentent pas. Nos agents écrivent aujourd'hui l'essentiel du code, alors j'ai cessé de défendre ces propriétés en relisant des diffs et j'ai commencé à les compiler. 67 décisions consignées, 39 contrôles qui font échouer la compilation, un verdict avant fusion, et exactement un périmètre où la réponse reste volontairement humaine. On mesure le résultat sur une seule échelle : jusqu'où un défaut va vers la droite avant que quelque chose l'arrête. Voici ce que j'ai construit, et la part qui ne prouve rien.</p></div>
<hr>
<p>Quand on me demande si je fais confiance au code que les agents de mon équipe écrivent, la réponse honnête est que la question est mal posée. Je ne lui fais pas confiance. Je ne le relis pas non plus, en tout cas plus ligne à ligne. Aucune de ces deux réponses n'apprend grand-chose.</p>
<p>Ce qui compte, c'est ce qu'on construit quand les deux sont vraies en même temps.</p>
...
<h2>Les 4 couches</h2>
<p>Ces propriétés ne reposent donc plus sur l'attention de quelqu'un, mais sur du code qui s'exécute. 4 couches, dans l'ordre où un changement les rencontre.</p>
