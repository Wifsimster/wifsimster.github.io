...
<p>The volume argument is real: more code now arrives per week than any team reads with genuine attention. Everyone has made that observation by now.</p>
...
<p>Where the obligation crosses files, human review is at its weakest and a script at its strongest. That has less to do with artificial intelligence than the framing usually suggests.</p>
<p>One finding did change how I read a proposal, though. A study accepted at <abbr title="Mining Software Repositories — a peer-reviewed software-engineering conference">MSR</abbr> 2026 measured that agent-written pull requests carry noticeably more redundant code, and that reviewers expressed <em>more</em> favourable sentiment toward them than toward human ones. Well-formed, idiomatic, generously commented code buys less suspicion per line than it has earned. It is plausible before it is correct, and plausibility is what a reviewer actually stops on.</p>
<p>The overclaim would be easy to make. This is not a new species of defect. A tired engineer forgets the same call on a Friday afternoon and always did. What changed is the rate at which the omission is produced, and the economics of catching it by reading.</p>

<h2>The 4 layers</h2>
<p>None of this is a verdict on the models. Those properties have to hold whoever wrote the line — an agent, a senior engineer, or me on a Friday evening. So they no longer rest on anyone's attention. They rest on code that runs. 4 layers, in the order a change meets them.</p>
...
<p><strong>One verdict before the merge.</strong> A single command, scoped to the diff, in parity with the pipeline, that prints PASS or FAIL. A proposal is opened only on PASS. This is the layer that made agent-written work reviewable at all. It does not prove the change is good; it converts "please have a look" into a claim someone can check.</p>
...
<p>A word on review, because the word misleads. On my team it is overwhelmingly automated: agents review the work of agents and file a report. The human does not read the lines. The human reads what happened, and owns what a machine cannot own — the decisions that are legally opposable, and the security trade-offs. Signing that this write must stay traceable for twenty years, or that a static entry secret deserves a second pair of eyes, is not proofreading. It is responsibility, and responsibility does not delegate to a model.</p>
<p>None of this exists to slow anyone down, and I would drop all of it tomorrow if it did. An executable rule is what lets us merge without a meeting: when the machine checks the constraint, nobody has to be convened to confirm it was respected. Speed and quality are not traded against each other here. They move together, and that is the only argument that survives the question of why we spend time writing checks at all.</p>
...
<p><strong>My confidence does not transfer.</strong> I know what the models I exercise daily produce. That knowledge does not exist for the person who joins on Monday, nor for a model I have not put through its paces, nor for me on a subject I know badly. The checks are what remains when my calibration is absent.</p>
...
<p>L'argument du volume est réel : il arrive chaque semaine plus de code qu'aucune équipe n'en lit avec une attention véritable. Tout le monde a fait ce constat.</p>
...
<p>Là où l'obligation traverse les fichiers, la revue humaine est au plus faible et un script au plus fort. Ce constat doit moins à l'intelligence artificielle que le cadrage habituel ne le laisse croire.</p>
<p>Un résultat a tout de même changé ma façon de lire une proposition. Une étude acceptée à <abbr title="Mining Software Repositories — conférence de génie logiciel à comité de lecture">MSR</abbr> 2026 mesure que les propositions écrites par des agents portent nettement plus de code redondant, et que les relecteurs y ont exprimé un sentiment <em>plus</em> favorable qu'envers celles des humains. Un code bien formé, idiomatique, généreusement commenté inspire moins de méfiance qu'il n'en mériterait. Il est plausible avant d'être juste, et c'est sur la plausibilité que le relecteur s'arrête.</p>
<p>Il serait tentant d'en faire une nouvelle espèce de défaut. Ce n'en est pas une. Un ingénieur fatigué oublie le même appel un vendredi après-midi, et l'a toujours fait. Ce qui a changé, c'est la cadence à laquelle l'omission est produite, et l'économie de sa détection par la lecture.</p>

<h2>Les 4 couches</h2>
<p>Rien de tout cela n'est un jugement sur les modèles. Ces propriétés doivent tenir quel que soit l'auteur de la ligne : un agent, un ingénieur chevronné, ou moi un vendredi soir. Elles ne reposent donc plus sur l'attention de quelqu'un, mais sur du code qui s'exécute. 4 couches, dans l'ordre où un changement les rencontre.</p>
...
<p>Un mot sur la revue, parce que le mot induit en erreur. Chez nous elle est très majoritairement automatisée : ce sont des agents qui relisent le travail des agents et qui en rendent un rapport. L'humain ne lit pas les lignes. Il lit ce qui s'est passé, et il assume ce qu'une machine ne peut pas assumer : les décisions opposables et les arbitrages de sécurité. Signer que cette écriture doit rester traçable vingt ans, ou qu'un secret d'entrée statique mérite un second regard, ce n'est pas de la relecture. C'est une responsabilité, et une responsabilité ne se délègue pas à un modèle.</p>
<p>Rien de tout cela n'existe pour ralentir, et j'abandonnerais l'ensemble demain si c'était le cas. Une règle exécutable est ce qui permet de fusionner sans réunion : quand la machine vérifie la contrainte, personne n'a besoin d'être convoqué pour confirmer qu'elle est respectée. La vélocité et la qualité ne s'échangent pas ici. Elles montent ensemble, et c'est le seul argument qui tient quand on demande pourquoi passer du temps à écrire des contrôles.</p>
...
<p>Nous mesurons sur une seule échelle : <strong>jusqu'où un défaut va vers la droite avant que quelque chose l'attrape.</strong> 6 étapes, dans l'ordre — la chaîne, la revue de code, le test manuel, la <abbr title="Quality Assurance — l'étape de qualification avant acceptation d'une livraison">recette</abbr>, le client pilote, la production. Chaque défaut porte l'étape qui l'a attrapé. Tout le reste en découle : densité de défauts rapportée aux propositions fusionnées, délai de correction par sévérité, taux de réouverture, backlog ouvert.</p>
...
<path d="M 640 268 L 96 268" fill="none" stroke="currentColor" stroke-opacity="0.5" stroke-width="1.5" stroke-linecap="round"></path>
<path d="M 108 260 L 96 268 L 108 276" fill="none" stroke="currentColor" stroke-opacity="0.5" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
<text x="96" y="250" fill="currentColor" font-family="ui-sans-serif, system-ui, sans-serif" font-size="14.5" font-weight="600" opacity="0.8">chaque règle exécutable déplace une classe d'un cran, définitivement</text>
...
<p><strong>Ma confiance ne se transmet pas.</strong> Je sais ce que produisent les modèles que j'éprouve quotidiennement. Cette connaissance n'existe pas chez la personne qui arrive lundi, ni pour un modèle que je n'ai pas encore éprouvé, ni pour moi sur un sujet que je maîtrise mal. Les contrôles sont ce qui reste quand mon étalonnage n'est pas là.</p>
<p><strong>Ça n'attrape pas tout, ça empêche seulement de retomber.</strong> 39 contrôles encodent 39 défauts que nous avons fini par trouver, la plupart après qu'ils nous ont coûté quelque chose. Chaque nouveau contrôle existe précisément parce que ce contrôle n'existait pas. Cela ne me dit pas que la chaîne est digne de confiance. Cela me dit que nous ne pouvons plus refaire 39 erreurs précises, et ne dit rien de la classe encore en cours que personne n'a remarquée.</p>
...
<p><strong>Les dernières étapes sont vides à cause d'où nous en sommes, pas de notre mérite.</strong> Nous ne sommes pas encore en production. La preuve la plus dure — ce qui survit au contact d'utilisateurs réels, sur des données hospitalières réelles, à volume réel — est donc une preuve que nous n'avons pas. Quiconque vous montre un tableau de détection avec les deux dernières colonnes vides vous montre un calendrier, pas un résultat qualité. Moi compris.</p>
