...
const englishContent: PostContent = {
  ...
  description: 'A merged fix, a green pipeline, a signed review — and the thing it guaranteed never happened once. The defect agents produce most often has no symptom, and no amount of rereading finds it. How the checks got written, one incident at a time, and the failure they will never catch.',
...
<hr>
<p>Six weeks after the change was merged, we found out the feature had never once done its job.</p>
<p>Not slowly. Not partially. Never. The change had been reviewed by a human, typechecked, unit-tested, run through our pre-merge gate and merged green. Every instrument we own said yes. And the thing it was supposed to guarantee — a regulatory obligation on <abbr title="Fictitious codename: the real project, its products and its vendor are anonymized.">Solstice*</abbr>, the hospital <abbr title="Enterprise Resource Planning — the administrative backbone of a hospital group">ERP</abbr> my team builds — had quietly not been happening since the day the change landed.</p>
<p>One thing matters for everything that follows: we are not in production yet. Solstice* is being built toward go-live. That is precisely why this defect could live for six weeks. No user calls a help desk about an audit trail that is not being written. Nothing in the system was ever going to tell us, and nothing did.</p>
...
<p><strong>The switch that switched nothing.</strong> Our configuration catalogue declared a flag as replacing an environment variable. The settings screen rendered the toggle. The service kept reading the environment variable. The screen told whoever set that toggle that calls to a national identity teleservice were now restricted. They were not. A guarantee you believe you have and do not is worth less than no guarantee at all, because you stop checking.</p>
...
<p>Deciding what a defect means. That 468 records are stalled is a technical fact. That it would break a twenty-year retention obligation the day we go live is a judgement, and it is the judgement that decides whether the fix lands tonight or next sprint.</p>
...
const frenchContent: PostContent = {
  ...
  description: "Un correctif fusionné, une CI verte, une revue signée — et la garantie qu'il devait produire, jamais rendue une seule fois. Le défaut que produisent le plus souvent nos agents n'a pas de symptôme, et le relire ne le trouve pas. Récit des garde-fous écrits un incident à la fois, et de ce qu'ils ne verront jamais.",
...
<p>Décider ce qu'un défaut signifie. Que 468 enregistrements soient bloqués est un fait technique. Que cela romprait une obligation de rétention à horizon vingt ans le jour de la mise en service est un jugement, et c'est ce jugement qui décide si le correctif part ce soir ou au prochain sprint.</p>
