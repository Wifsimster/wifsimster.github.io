...
<p>So the properties moved out of people's attention and into things that execute. Four layers, in the order a change meets them.</p>
<p><strong>Recorded decisions.</strong> Sixty-seven architecture decisions, dated and versioned in the repository. They carry the <em>reason</em>, which is the part an agent otherwise refactors away because it looks like an accident. Every agent reads them before touching anything structural. No human on the team reads all sixty-seven. I do not read all sixty-seven.</p>
<p><strong>Executable rules.</strong> Thirty-nine checks that fail the build, inside forty-four continuous-integration jobs. Each one encodes one property, and each exists because we got it wrong once. They share a single design rule, and it is the only genuinely transferable thing in this post: <strong>derive the perimeter from the code, never recopy it</strong>. A hand-maintained list of paths is wrong one rename later, and it fails silently, which is worse than failing loudly.</p>
...
<svg viewBox="0 0 800 340" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="The quality system has four layers, each narrower than the last: sixty-seven recorded architecture decisions, thirty-nine checks that fail the build, one pre-merge PASS or FAIL verdict, and one perimeter deliberately reserved for human review." class="diagram-svg">
...
<text x="64" y="76" fill="currentColor" font-family="ui-sans-serif, system-ui, sans-serif" font-size="28" font-weight="650">Four layers. Only one is a person.</text>
...
<svg viewBox="0 0 800 340" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Defects are classified by the stage that caught them, across six rungs from the build pipeline through code review, manual testing, QA and the pilot site to production; the last two rungs are counted as escaped defects, and every executable rule added moves a class of defect permanently leftward." class="diagram-svg">
...
<p><strong>It is a ratchet, not a net.</strong> Thirty-nine checks encode thirty-nine defects we eventually found, most after they had already cost us something. Each new check exists precisely because that check did not exist. This does not tell me the pipeline is trustworthy. It tells me we cannot repeat thirty-nine specific mistakes, and it says nothing at all about the class still running that nobody has noticed.</p>
...
<p><strong>The escape hatches are not counted.</strong> A rule that can never be waived gets worked around, so ours carry greppable derogations with a written reason beside each. About a dozen are in use. But exactly one check counts its derogations over time against a baseline that can only go down. The rest could accumulate exemptions quietly, and nothing prevents an agent from writing its own exemption to turn itself green. That is the open flaw in everything above, and I do not have a fix shipped for it.</p>
...
<ol>
<li><strong>Write down the properties, not the process.</strong> Five sentences that must be true after every change. If you cannot name them, no amount of tooling will protect them.</li>
...
<li><a target="_blank" rel="noopener noreferrer" href="https://arxiv.org/abs/2211.03622">Perry et al., Do Users Write More Insecure Code with AI Assistants? (ACM CCS 2023)</a>: assisted participants wrote less secure code and were more confident it was secure. Forty-seven participants, lab setting, a model three generations old. Cite it for the mechanism, never as a current rate.</li>
...
const frenchContent: PostContent = {
  ...
  description: "Peut-on faire confiance au code écrit par des agents les yeux fermés ? Non. Mais le relire ligne à ligne n'est pas davantage la solution. Voici le système qualité que j'ai construit sur un ERP hospitalier régulé : ce que qualité veut dire quand le logiciel est opposable, les quatre couches qui la tiennent, l'échelle sur laquelle on la mesure, et ce que tout cela ne prouve pas.",
  html: `<div class="post-tldr"><p><strong>TL;DR</strong> En santé régulée, la qualité n'est pas un nombre de bogues. C'est un jeu de propriétés qui doivent tenir après chaque changement : une trace d'audit opposable, une identité que personne ne peut forger, l'accessibilité comme obligation légale, des contrats d'interopérabilité qui ne mentent pas. Nos agents écrivent aujourd'hui l'essentiel du code, alors j'ai cessé de défendre ces propriétés en relisant des diffs et j'ai commencé à les compiler. Soixante-sept décisions consignées, trente-neuf contrôles qui font échouer la compilation, un verdict avant fusion, et exactement un périmètre où la réponse reste volontairement humaine. On mesure le résultat sur une seule échelle : jusqu'où un défaut va vers la droite avant que quelque chose l'arrête. Voici ce que j'ai construit, et la part qui ne prouve rien.</p></div>
<hr>
...
<p>Les propriétés ont donc quitté l'attention des gens pour rejoindre des choses qui s'exécutent. Quatre couches, dans l'ordre où un changement les rencontre.</p>
<p><strong>Les décisions consignées.</strong> Soixante-sept décisions d'architecture, datées et versionnées dans le dépôt. Elles portent la <em>raison</em>, c'est-à-dire la partie qu'un agent supprime au premier remaniement parce qu'elle ressemble à un accident. Chaque agent les lit avant de toucher à quoi que ce soit de structurant. Aucun humain de l'équipe ne lit les soixante-sept. Moi non plus.</p>
<p><strong>Les règles exécutables.</strong> Trente-neuf contrôles qui font échouer la compilation, à l'intérieur de quarante-quatre travaux d'intégration continue. Chacun encode une propriété, et chacun existe parce que nous nous sommes trompés une fois. Ils partagent une seule règle de conception, et c'est la seule chose vraiment transposable de ce billet : <strong>dériver le périmètre du code, jamais le recopier</strong>. Une liste de chemins tenue à la main est fausse un renommage plus tard, et elle échoue en silence, ce qui est pire qu'échouer bruyamment.</p>
...
<svg viewBox="0 0 800 340" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Le système qualité compte quatre couches, chacune plus étroite que la précédente : soixante-sept décisions d'architecture consignées, trente-neuf contrôles qui font échouer la compilation, un verdict PASS ou FAIL avant fusion, et un périmètre volontairement réservé à la revue humaine." class="diagram-svg">
...
<text x="64" y="76" fill="currentColor" font-family="ui-sans-serif, system-ui, sans-serif" font-size="28" font-weight="650">Quatre couches. Une seule est humaine.</text>
<g stroke="currentColor" stroke-opacity="0.22" stroke-width="1">
<rect x="64" y="100" width="672" height="46" rx="8" fill="currentColor" fill-opacity="0.12"></rect>
<rect x="64" y="156" width="520" height="46" rx="8" fill="currentColor" fill-opacity="0.18"></rect>
<rect x="64" y="212" width="368" height="46" rx="8" fill="currentColor" fill-opacity="0.24"></rect>
...
<svg viewBox="0 0 800 340" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Les défauts sont classés par l'étape qui les a attrapés, sur six barreaux allant de la chaîne de compilation à la production en passant par la revue, le test manuel, la recette et le client pilote ; les deux derniers barreaux comptent comme défauts échappés, et chaque règle exécutable ajoutée déplace une classe de défauts définitivement vers la gauche." class="diagram-svg">
...
<p><strong>C'est un cliquet, pas un filet.</strong> Trente-neuf contrôles encodent trente-neuf défauts que nous avons fini par trouver, la plupart après qu'ils nous aient coûté quelque chose. Chaque nouveau contrôle existe précisément parce que ce contrôle n'existait pas. Cela ne me dit pas que la chaîne est digne de confiance. Cela me dit que nous ne pouvons plus refaire trente-neuf erreurs précises, et ne dit rien de la classe encore en cours que personne n'a remarquée.</p>
...
<ol>
<li><strong>Écrivez les propriétés, pas le processus.</strong> Cinq phrases qui doivent être vraies après chaque changement. Si vous ne savez pas les nommer, aucun outillage ne les protégera.</li>
...
<li><a target="_blank" rel="noopener noreferrer" href="https://arxiv.org/abs/2211.03622">Perry et coll., Do Users Write More Insecure Code with AI Assistants ? (ACM CCS 2023)</a> : les participants assistés ont écrit du code moins sûr et étaient plus convaincus qu'il l'était. Quarante-sept participants, cadre de laboratoire, modèle vieux de trois générations. À citer pour le mécanisme, jamais comme un taux actuel.</li>
