  ...
  <div>
    ...
    <div>
      <div v-for="(postsByYear, year) in groupedPosts" :key="year" class="mb-12">
        <h2 class="text-3xl font-bold text-gray-900 dark:text-zinc-100 mb-6">{{ year }}</h2>
        <div v-for="(postsByMonth, month) in postsByYear" :key="month" class="mb-8">
          <h3 class="text-xl font-semibold text-gray-700 dark:text-zinc-300 mb-4">{{ month }}</h3>
...
const i18n = useI18n()
const { getArchivedPosts } = usePosts()
